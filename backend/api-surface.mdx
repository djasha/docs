...
The safe setup payload includes a human guide URL, the public DealDash Agent Kit repo URL, an agent-optimized docs URL, the MCP command/config path, and the official MCP SDK URL. Public and browser-visible responses expose names and links only, never secret values.
