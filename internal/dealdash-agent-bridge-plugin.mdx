...
Photo uploads accept PNG, JPEG/JPG, WebP, GIF, AVIF, and BMP raster images by default. The MCP bridge enforces `DEALDASH_AGENT_MAX_IMAGE_BYTES` before sending the payload to DealDash. The current default binary image limit is 25MB; the DealDash API JSON body limit has 50MB headroom for base64 encoding. Raise those limits only in controlled deployments because base64 uploads are memory-heavy.
