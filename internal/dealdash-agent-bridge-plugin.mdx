...
Render MCP is a separate optional infrastructure plugin. Use [Add Render MCP](../agents/render-mcp-setup.mdx) or `/tools/render-mcp` only for Render services, deploys, logs, metrics, databases, and environment-variable troubleshooting. Do not use Render MCP for DealDash app data.
